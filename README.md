...
This also doesn't need any imports to work. Just paste the script here's an example of how to use it:
...
Anyway, here's the function, just paste it where you need it:
"""
...
def injectinator(func):
    def wrapper(*args, **kwargs):
        replacements = dict(zip(
            ...
            list(([None] * func.__code__.co_argcount) + list(func.__defaults__))[-func.__code__.co_argcount:]
        ))
